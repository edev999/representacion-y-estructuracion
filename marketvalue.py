...
def main():
    ...
    df_merged = pd.merge(df, df_stats,
    ...
    print(f"Jugadores combinados con solo columnas numéricas y Transfermarkt: {len(df_merged_reducido)}")
    print(df_merged_reducido.head())

    # Guardar resultado combinado
    df_merged_reducido.to_csv("data_output/datos_jugadores.csv", encoding="utf-8-sig", index=False)
